...
import { generateTasks as gt, breakdownTask as bt, summarizeBoard as sb } from "../services/aiService.js";
...
export const generateTasks = asyncHandler(async (req, res) => {
    ...
    const suggestions = await gt(goal, count);
...
export const breakdownTask = asyncHandler(async (req, res) => {
    ...
    const subtasks = await bt(title, description, count);
    res.json({ subtasks });
});
