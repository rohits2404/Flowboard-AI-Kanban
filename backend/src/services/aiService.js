...
const getClient = () => {
    ...
    if (!key || key === "your-groq-api-key") {
        throw new ApiError(503, "Groq API key is not configured on the server");
    }
    if (!client) {
        client = new OpenAI({
            apiKey: key,
...
const runPrompt = async (prompt) => {
    try {
        ...
        if (status === 429) {
            throw new ApiError(
                429,
                "AI quota exceeded. Check your Groq plan/billing and try again later."
            );
        }
        if (status === 400 || status === 401 || status === 403) {
            throw new ApiError(
                503,
                "AI request rejected - verify your GROQ_API_KEY is valid."
            );
        }
