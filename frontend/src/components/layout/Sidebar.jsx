...
export const Sidebar = ({ collapsed, onToggle, onCreateBoard, onCommand }) => {
    ...
    return (
        <aside
            className={cn(
                "fixed inset-y-3 left-3 z-40 hidden flex-col overflow-hidden rounded-3xl border border-line bg-surface/90 shadow-soft backdrop-blur-xl transition-[width] duration-300 ease-spring md:flex",
                collapsed ? "w-18" : "w-63"
            )}
        >
            {/* Header */}
            <div className="flex h-16 items-center gap-2.5 px-3.5">
                <div className="brand-gradient grid h-10 w-10 shrink-0 place-items-center rounded-2xl shadow-(--shadow-brand)">
