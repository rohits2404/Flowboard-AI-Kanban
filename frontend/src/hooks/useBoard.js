...
export const useBoard = (boardId) => {
    ...
    useEffect(() => {
        ...
        const onColCreated = (c) =>
            setColumns((p) =>
                [...p, c].sort((a, b) => a.position - b.position)
            );
        ...
        const onPresenceJoin = ({ user }) =>
            setPresence((p) =>
                p.find((u) => u.id === user.id) ? p : [...p, user]
            );
    ...
    const addColumn = useCallback(
        async (title) => {
            try {
                ...
                setColumns((p) =>
                    [...p, col].sort((a, b) => a.position - b.position)
                );
            } catch (err) {
                toast.error(err.message);
            }
    ...
    const deleteColumn = useCallback(
        async (columnId) => {
            try {
                ...
                setColumns((p) =>
                    p.filter((c) => c.id !== columnId)
                );

                setTasks((p) =>
                    p.filter((t) => t.column_id !== columnId)
                );
            } catch (err) {
                toast.error(err.message);
            }
