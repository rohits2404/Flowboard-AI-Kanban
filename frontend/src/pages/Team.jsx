...
const Team = () => {
	...
	return (
		<>
			<Topbar
				title="Team"
				subtitle="People across your boards"
					...
					<div className="mb-6 flex flex-wrap items-center gap-3">
						...
						<div className="relative ml-auto">
							...
							<input
								...
								className="h-9 w-56 rounded-full border border-line bg-surface pl-9 pr-4 text-xs shadow-(--shadow-card) outline-none transition-all duration-200 focus:border-brand-500/50 focus:ring-2 focus:ring-brand-500/15"
							/>
						</div>
					</div>
					...
					{loading ? (
						...
						<div className="card flex flex-col items-center justify-center gap-3 rounded-3xl py-20 text-center">
							...
							<div>
								...
								<p className="mt-1.5 max-w-sm text-sm leading-relaxed text-muted">
									Invite people to a board and they’ll appear
									here.
								</p>
							</div>
						</div>
						...
						<div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
							{filtered.map((m) => (
								<div
									key={m.id}
									className="flex flex-col items-center rounded-3xl border border-line bg-surface p-6 text-center shadow-(--shadow-card) transition-shadow duration-300 hover:shadow-(--shadow-soft)"
								>
									<Avatar
										name={m.name}
										id={m.id}
