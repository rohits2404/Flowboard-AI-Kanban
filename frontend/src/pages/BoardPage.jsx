...
const BoardPage = () => {
	...
	const handleBreakdown = async (task) => {
		try {
			const subtasks = await aiApi.breakdown(boardId, {
				taskId: task.id,
			});
	...
	const actions = (
		<div className="flex items-center gap-2">
			...
			<Button
				size="sm"
				onClick={() =>
					setAiGen({ open: true, columnId: b.columns[0]?.id })
				}
			>
			...
			<div className="flex flex-wrap items-center gap-2.5 px-6 py-3.5">
				<div className="relative">
					...
					<input
						...
						className="h-9 w-52 rounded-full border border-line bg-surface pl-9 pr-4 text-xs shadow-(--shadow-card) outline-none transition-all duration-200 focus:border-brand-500/50 focus:ring-2 focus:ring-brand-500/15"
					/>
				</div>
				<FilterSelect
