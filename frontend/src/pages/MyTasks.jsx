...
const MyTasks = () => {
	...
	const mine = useMemo(
		() =>
			tasks.filter(
				(t) => t.assignee_id && t.assignee_id === user?.id
			),
		[tasks, user]
	);
				...
				<div className="mx-auto max-w-[1600px] px-6 py-8 md:px-8">
					...
					<div className="mb-6 flex flex-wrap items-center gap-2.5">
						<input
							...
							className="h-9 w-56 rounded-full border border-line bg-surface px-4 text-xs shadow-(--shadow-card) outline-none transition-all duration-200 focus:border-brand-500/50 focus:ring-2 focus:ring-brand-500/15"
						/>
