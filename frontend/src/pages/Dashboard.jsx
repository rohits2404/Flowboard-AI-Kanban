...
const TasksByBoard = ({ boards, className }) => {
	...
	return (
		<div
			className={cn(
				"flex flex-col rounded-3xl border border-line bg-surface p-6 shadow-(--shadow-card)",
				className,
			)}
		>
			<SectionTitle
				icon={BarChart3}
				hint="Tasks across your busiest boards"
			>
				Board analytics
			</SectionTitle>
			{hasTasks ? (
...
const WorkspaceDonut = ({ owned, shared, boards = [], className }) => {
	...
	const busiest = boards.reduce(
		(a, b) =>
			Number(b.task_count || 0) > Number(a?.task_count || 0) ? b : a,
		null,
	);
...
const AIPromo = ({ onCreate, className }) => (
	<div
		className={cn(
			"brand-gradient relative flex flex-col justify-between overflow-hidden rounded-3xl p-6 text-white shadow-(--shadow-brand)",
			className,
		)}
	>
		...
		<div className="relative">
			...
			<p className="mt-1.5 text-sm leading-relaxed text-white/80">
				Spin up a board and turn a one-line goal into a prioritized
				backlog in seconds.
			</p>
		</div>
		<button
	...
	<div className="card flex flex-col items-center justify-center gap-4 rounded-3xl py-20 text-center">
		...
		<div>
			...
			<p className="mt-1.5 max-w-sm text-sm leading-relaxed text-muted">
				Spin up a board and let AI generate your first set of tasks from
				a simple goal.
			</p>
		</div>
